...
class Mutator:
    ...
    def mutate(self, gene):
        ...
        if mutationType == 0:
            gene.x = self._bound(gene.x + random.randint(0,maxDelta) - halfDelta, 0, self.width)
        elif mutationType == 1:
            gene.y = self._bound(gene.y + random.randint(0,maxDelta) - halfDelta, 0, self.height)
        elif mutationType == 2:
            gene.z = random.randint(0, 500)
        elif mutationType == 3:
            gene.r = self._bound(gene.r + random.randint(0, maxDelta) - halfDelta, 5, int(min(self.width,self.height)/5))
        else:
